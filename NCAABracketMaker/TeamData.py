...
def getTeamList(league):
    ...
    for team in id_div:
        team = str(team).replace(
            f'<a class="AnchorLink" href="/{league}-college-basketball/team/schedule/_/id/',
            "",
        )
...
def getTeamData(league, year):
    ...
    for teamid in teamIDs:
        ...
        schedule_div = soup.find_all(
            "tr", attrs={"class": re.compile("Table__TR Table__TR--sm Table__even")}
        )
        gameCount = 1
        teamSchedule = {}
        winRecord = 0
        ...
        if teamSchedule == {}:
            ...
            team_div = soup.find_all(
                "div",
                class_="ClubhouseHeader__Main flex items-center pv3 justify-start",
            )
...
def nameCheck(teamName):
    ...
    if team.split(" ")[0] == "St":
        if (
            team.split(" ")[1] == "Peters"
            or team.split(" ")[1] == "Josephs"
            or team.split(" ")[1] == "Louis"
            or team.split(" ")[1] == "Marys"
        ):
            team = (
                team.replace("St", "Saint")
                .replace("Peters", "Peter's")
                .replace("Josephs", "Joseph's")
                .replace("Marys", "Mary's")
            )
        else:
...
def getemptybracket(league, testyear):
    ...
    year = (
        str(year)
        .replace('<h1 class="h2">NCAA Tournament Bracket - ', "")
        .replace("</h1>", "")
    )
    ...
    for element in seed_list:
        bracket_teams[
            f'd1r{-(-(seed_list.index(element)+1)//16)}seed{element.split(" ")[0]}'
